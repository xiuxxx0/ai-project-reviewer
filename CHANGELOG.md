...
- 出题/阅卷 JSON 解析失败自动纠错重试一次，并加大出题 max_tokens（推理模型的思考过程不再截断 JSON）

## [0.1.3] - 2026-08-15

### 修复

- LLM 请求代理容错：系统代理（如 Clash）未运行时自动直连重试，不再报 WinError 10061
- 所有 LLM 板块失败时 CLI 明确警告并提示排查方向

## [0.1.2] - 2026-08-15

### 修复

- 版本号单一来源：\_\_version\_\_ 改为从包元数据读取（此前 PyPI 0.1.1 的横幅仍显示 0.1.0）
